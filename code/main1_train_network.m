...
lead_num_notes = max(lead) - offset + 1; % TODO: why bass offset?
num_dur = 16;
...
if (flag_rand == 1)
  ...
  W_in_raw(1:network_size, 2:7) = 0.8 * (2 * rand(network_size, 6) - 1);
  W_in_raw(1:network_size, 8:13) = (2 * rand(network_size, 6) - 1);
  ...
  W_raw = 2 * rand(network_size, network_size) - 1;
  % randomly generate feedback weights
  %W_fb_raw = rand(network_size, output_length);
  W_fb_raw = 2 * rand(network_size, output_length) - 1;
end
...
for i = 1:timesteps-1
  X(i+1, :) = (1-a_leak) * X(i,:)' + a_leak * tanh(W * X(i, :)' + W_in * u_normalized(i+1, :)' + W_fb * D(i, :)'); % + W_fb * D(i, :)'
end
